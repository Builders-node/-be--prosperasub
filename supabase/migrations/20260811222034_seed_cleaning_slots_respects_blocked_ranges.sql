...
-- Applied 20260811222034 · seed_cleaning_slots_respects_blocked_ranges

-- The generated grid must agree with the preview the provider is looking at.
--
-- Two things were wrong once blocked ranges existed:
--
--   1. The seeder did not read them at all, so it published slots inside a
--      lunch break. The booking page filtered those out at read time, which
--      hid them but left the day short.
--   2. Both engines now resume a day AT the moment a block ends — the buffer
--      is the gap after a JOB, and a blocked hour is not one — while the
--      seeder kept stepping its fixed grid, so a 12:00–15:00 break was
--      followed by 15:30 instead of 15:00. The provider's own preview would
--      have promised an hour no row existed for.
--
-- That makes the inner generation procedural: a set-based grid cannot express
...
-- to before — verified by re-running and finding nothing new to insert.
--
