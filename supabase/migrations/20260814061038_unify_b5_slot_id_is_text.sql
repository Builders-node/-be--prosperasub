...
-- Applied 20260814061038 · unify_b5_slot_id_is_text

-- `cleaning_available_slots.id` is a readable string, not a uuid
-- ("owned-cleaning-slot-2026-08-08-1200"). Assigning it to a uuid variable is
-- what the mirror was failing on — silently, because the handler that stops
-- the mirror from breaking a real booking also hides why it wrote nothing.
alter table service_occurrences alter column slot_id type text using slot_id::text;
