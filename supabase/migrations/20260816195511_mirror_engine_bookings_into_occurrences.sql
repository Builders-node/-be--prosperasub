...
-- Applied 20260816195511 · mirror_engine_bookings_into_occurrences

-- The beach club's day was always empty.
--
-- `service_occurrences` is mirrored from the legacy tables, and the beach's
-- entry was `beach_club_court_bookings` — which has been empty since the
-- booking engine took the traffic. Eleven booked hours live in `bookings` and
-- none of them reached the one table the provider's Operations tab reads.
--
-- Same mirror function, one more branch: the engine's table is simply where
-- this service's facts live now.
create or replace function public.mirror_legacy_occurrence()
returns trigger
language plpgsql
security definer
...
begin
  if TG_TABLE_NAME = 'cleaning_bookings' then
    ...
    -- The engine's own table: an hour on a resource, for whatever provider
    -- owns it. Not beach-specific — any provider with a calendar lands here.
    v_svc := 'beach';
    v_provider := new.provider_id;
    v_resource := new.resource_id;
    v_start := new.start_at;
    v_end := new.end_at;
    v_status := case lower(coalesce(new.status, ''))
                  when 'cancelled' then 'cancelled' when 'canceled' then 'cancelled'
                  when 'completed' then 'done' when 'no_show' then 'failed'
                  when 'expired' then 'cancelled'
                  else 'scheduled' end;
    -- "user:<uuid>" is how the engine names whoever booked it.
    v_user_raw := nullif(regexp_replace(coalesce(new.subject_ref, ''), '^user:', ''), '');
    -- The court's name, so a row reads "Tennis Court 1" the way a food row
    -- reads "Lunch".
    select r.name into v_item from bookable_resources r where r.id = new.resource_id;
    v_notes := coalesce(new.notes, new.label);
    v_sub := new.order_ref;
    v_gcal_id := new.google_calendar_event_id; v_gcal_stat := new.google_calendar_sync_status;
...
drop trigger if exists mirror_booking_occurrence on public.bookings;

create trigger mirror_booking_occurrence
  after insert or update on public.bookings
  for each row execute function public.mirror_legacy_occurrence();
