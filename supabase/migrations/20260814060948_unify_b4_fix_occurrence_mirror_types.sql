...
-- Applied 20260814060948 · unify_b4_fix_occurrence_mirror_types

-- Three type mismatches the first version swallowed, because the handler that
-- keeps the mirror from breaking a real booking also hides why it produced
-- nothing:
--   • `cleaning_available_slots.start_time` / `end_time` are TEXT, so
--     `date::timestamp + start_time` is `timestamp + text` — no such operator.
--   • legacy `user_id` is TEXT and not always a uuid ("admin-custom-cleaning"
--     was a real value), while the universal layer is uuid.
--   • `food_delivery_logs` has no user at all; it comes from the subscription.
...
begin
  if TG_TABLE_NAME = 'cleaning_bookings' then
    ...
    -- plan (`lead_time_minutes` / `window_minutes`), applied in phase C.
  ...
  if v_provider is null or v_start is null then
    -- Nothing to attach it to, or no time to put it at. Skipping beats filing
    -- an occurrence under the wrong business or at the wrong hour.
    return new;
  end if;

  -- Legacy ids are text and were not always uuids. A non-uuid is dropped
  ...
  if v_existing is not null then
    update service_occurrences set
      provider_id = v_provider, resource_id = v_resource, user_id = v_user,
      source_subscription_id = v_sub,
      starts_at = v_start, ends_at = v_end, slot_id = v_slot,
