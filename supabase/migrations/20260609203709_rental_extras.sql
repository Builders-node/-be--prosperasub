...
-- Applied 20260609203709 · rental_extras
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_rental_extras_updated_at') THEN
    CREATE TRIGGER trg_rental_extras_updated_at
      BEFORE UPDATE ON public.rental_extras FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
  END IF;
END$$;

INSERT INTO public.rental_extras (name, price_cents, price_type, sort_order)
SELECT * FROM (VALUES
  ('Theft Protection',          500,  'per_day', 1),
  ('Glass & Tyre Protection',   500,  'per_day', 2),
  ('Additional Driver',         500,  'per_day', 3),
  ('Baby Seat',                 400,  'per_day', 4),
  ('Cooler',                    300,  'per_day', 5),
  ('Tank Fill',                 1000, 'flat',    6),
  ('Custom Carwash',            2000, 'flat',    7),
  ('Tyre Change Service',       0,    'flat',    8)
