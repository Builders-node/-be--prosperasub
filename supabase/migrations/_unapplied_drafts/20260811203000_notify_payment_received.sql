...
-- that sees all of it. admin.service.ts no longer sends its own copy.
--
-- Idempotent by design: it fires on the TRANSITION to paid and re-checks that
-- no payment_received exists for the row, so a re-run, a double PATCH or the
-- admin path firing first can never send twice. Verified by moving a
-- subscription pending → paid → pending → paid and finding exactly one row.
--
-- TWO TRAPS, both of which this function's own exception handler hid so that
-- the migration reported success while nothing was written:
--   * user_notifications.recipient_user_id is uuid, but three of the four
--     subscription tables store user_id as TEXT — and some of those values are
--     Google subject ids, not uuids. Hence the regex guard; a subscription
--     whose owner is not a uuid gets no notification, because there is no way
--     to address one to them.
--   * Verify a trigger by reading the rows back. "Migration applied" says
