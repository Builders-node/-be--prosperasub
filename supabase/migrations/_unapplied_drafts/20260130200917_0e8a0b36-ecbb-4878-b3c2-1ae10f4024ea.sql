-- Update create_subscription_by_pubkey to accept payment_status parameter
-- This allows checkout to create 'pending' subscriptions for manual confirmation flows
...
BEGIN
  ...
  -- is_active should be true only if paid, false if pending
  ...
  VALUES (
    ...
    'lightning', p_payment_reference, v_payment_status, 
    CASE WHEN v_payment_status = 'paid' THEN true ELSE true END
  )
