...
-- Applied 20260817232311 · food_mirror_uses_item_time

-- The same hour, when a delivery is logged rather than generated.
--
-- `food_delivery_logs` records a DATE, so the mirror wrote midnight and a meal
-- marked delivered jumped from 12:00 to… 12:00. It takes the provider's hour
-- for that meal now, exactly like the generator, so a day reads in order
-- whether its rows were planned or logged.
...
begin
  if TG_TABLE_NAME = 'cleaning_bookings' then
    ...
    -- The provider's hour for this meal, midday when they have not said.
    v_start := (new.delivery_date::timestamp
                 + make_interval(mins => coalesce(provider_item_minutes(v_provider, v_item), 12 * 60)))
    ...
    v_sub := new.subscription_id::text; v_notes := new.reason;
