...
-- landed in storage, and the picture was never seen. When this was applied no
-- provider had an avatar in either id-space, which is exactly what a silently
...
-- Verified by writing an avatar to cleaning_providers the way the Info tab
-- does and watching it appear on the matching providers row.
...
-- Whatever is already there (nothing when applied) travels across once.
update providers p
